...
interface CircularGalleryProps
  ...
  scrollEase?: number;
  /**
   * Velocity of auto-scroll when not interacting.
   * @default 0.01
   */
  autoScrollSpeed?: number;
  /**
   * Delay in milliseconds before auto-scroll resumes after interaction.
   * @default 2000
   */
  resumeDelay?: number;
  /**
...
class Media {
  ...
  update(
    ...
    this.speed = (scroll.current - scroll.last) * 2;
...
class App {
  ...
  createMedias(
    ...
    const defaultItems: GalleryItem[] = [
      { image: `https://picsum.photos/seed/1/800/600?grayscale`, text: "Fellowship" },
    ];
  ...
  update() {
    // Auto-scroll logic with resume delay
    const now = Date.now();
    const timeSinceLastInteraction = now - this.lastInteractionTime;
    
    if (!this.isDown && timeSinceLastInteraction > this.resumeDelay) {
...
const CircularGallery = ({
  ...
  useEffect(() => {
    ...
    const computedFontSize = computedStyle.fontSize || "32px";
    const computedFontFamily = computedStyle.fontFamily;

    // We want a very premium, bold look for the labels
  ...
  }, [items, bend, borderRadius, scrollSpeed, autoScrollSpeed, resumeDelay, scrollEase, fontClassName]);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "text-brand-sage font-black text-[32px] tracking-tighter uppercase italic",
        fontClassName,
        className,
      )}
      {...props}

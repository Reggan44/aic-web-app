    ...
    useGSAP(
      () => {
        ...
        if (!prefersReducedMotion) {
          // 1. Entrance animation (still triggered by scroll, but no pinning)
          gsap.fromTo(
          ...
          // 2. Horizontal Interaction Logic
          let rotation = 0;
          const snapDegree = 360 / childrenCount;

          const updateRotation = (delta: number) => {
            rotation += delta * 0.1; // Sensitivity
          ...
          const snapToItem = () => {
            const snappedRotation = Math.round(rotation / snapDegree) * snapDegree;
          ...
          Observer.create({
            ...
            onChange: (self) => {
              // Only trigger on horizontal movement
              if (Math.abs(self.deltaX) > Math.abs(self.deltaY)) {
                updateRotation(direction === 'ltr' ? self.deltaX : -self.deltaX);
              }
            },
            onStop: () => {
              snapToItem();
            ...
            tolerance: 10,
            preventDefault: false, // CRITICAL: Allow vertical scroll to pass through
          });
          
          // Set initial cursor
          pinRef.current.style.cursor = 'grab';
        }
      },
      {
        scope: pinRef,

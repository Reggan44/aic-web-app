        ...
        <AnimatePresence mode="wait">
          {view === 'list' ? (
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                {selectedBook && Array.from({ length: selectedBook.chapters }, (_, i) => i + 1).map(chapter => (
                  <button
                    ...
                    className="aspect-square flex flex-col items-center justify-center border border-slate-100 rounded-2xl hover:border-brand-sage hover:bg-brand-sage/5 transition-all group relative overflow-hidden"
                  >
                    <div className="absolute inset-0 bg-brand-sage/5 scale-0 group-hover:scale-100 transition-transform duration-500 rounded-full" />
                    <span className="text-lg font-light text-brand-grey group-hover:text-brand-sage transition-colors relative z-10" style={{ fontFamily: "'Montserrat', sans-serif" }}>{chapter}</span>
                  </button>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div 
              ...
              exit={{ opacity: 0, y: 20 }}
              className="mt-[140px] md:mt-[180px]"
            >
              {loading ? (
                <div className="flex flex-col items-center justify-center py-48 gap-6">
                  <Loader2 size={48} className="animate-spin text-brand-sage/40" />
